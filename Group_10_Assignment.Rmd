...
listings <- listings %>%
  mutate(prop_type_simplified = case_when(
    property_type %in% c("Entire rental unit","Private room in rental unit", "Room in boutique hotel","Entire serviced apartment") ~ property_type, 
    TRUE ~ "Other"
  ))
listings %>%
...
round(mosaic::favstats(unlist(new_listings ["review_scores_rating"])), digits = 2)
...
new_listings %>% 
  ...
  arrange(desc(percentage))
```
There are 4 types of rooms, "Entire home", "Private room", "Hotel room", and "Shared room". Approximately 70% of listings are in the "Entire home/apt" category.
```{r fig.width = 12, fig.height = 12}
...
new_listings %>% 
  ...
  arrange(desc(percentage))
```
As expected, neighborhood is an important factor determining the price of the property. Majority of the properties are in Beyoglu & Fatih which are the most touristic regions of Istanbul. 
...
model_test<- lm(price_4_nights ~ bathrooms + bedrooms + beds + accommodates, data = four_nights_for_two_listings)
...
correlation_check <- four_nights_for_two_listings %>% 
  select(price_4_nights, bathrooms, bedrooms, beds, accommodates)%>% 
  ggpairs()
...
east <- c("Pendik","Tuzla","Cekmekoy","Sancaktepe","Sile","Umraniye","Kartal","Maltepe")
central_west <- c("Beyoglu","Sariyer","Sisli", "Besiktas","Kagithane")
central_east <- c("Uskudar","Kadikoy","Beykoz","Adalar","Atasehir")
west_south <- c("Bagcilar","Basaksehir","Bakirkoy","Bahcelievler","Zeytinburnu","Bayrampasa","Gungoren","Gaziosmanpasa","Esenler")
west <- c("Esenyurt","Avcilar","Silivri","Buyukcekmece","Kucukcekmece","Catalca","Beylikduzu")
...
```{r}
model3 <- lm(price_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating + instant_bookable + neighbourhood_simplified, data = four_nights_for_two_listings)
...
correlation_check_availability_reviewspcm <- four_nights_for_two_listings %>% 
  select(price_4_nights, availability_30, reviews_per_month)%>% 
  ggpairs()
...
```{r}
model4 <- lm(price_4_nights ~ prop_type_simplified + number_of_reviews + 
...
```{r}
model5 <- lm(price_4_nights ~ prop_type_simplified + number_of_reviews + 
...
```{r}
#Filter a private room, at least 10 reviews, average rating of at least 90% of 5(since the rating is from 0 to 5, not 0 to 100)
...
ggplot(model_predictions, aes(x = neighbourhood_simplified, y = .fitted)) +
  geom_line() +
