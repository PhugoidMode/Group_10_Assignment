...
- We analysed rent prices for AirBnb flats in Istanbul
...
- The selected variables had limited relationship with prices per night as shown by relatively low correlation
- The model that produced the has the highest predictability of prices per night includes the type of property, number of reviews, review scores rating, room type, availability over the next 30 days, and neighborhood
- Having used logarthmic price to carry out the regression, we ultimately achieved amn adjusted $R^2$ of 0.286, and predict (with a 95% confidence level) that the price of a 4 night stay in Istanbul will be 
...
We want to determine if `room_type` is a significant predictor of the cost for 4 nights, given everything else in the model, so we will fit a regression model called model2 that includes all of the explananatory variables in `model1` plus `room_type`. 
```{r}
...
Our dataset has many more variables, so we will be extending our analysis by exploring how further variables, such as neighbourhood, affect price.
...
Several factors can influence price, including the neighbourhood of the property, its availability, and review history. We will add another model, model3, to further explore this relation between neighbourhood and price:
...
```
Seem fine so adding these to a fourth model, this time leaving out the neighbourhood variable and including availability: 
...
Having built our models, we will check for any issues by plotting residuals etc, thus checking if there are outlier issues:
...
Istanbul shows a unique set of characteristics in that the regression models show price dropping as ratings etc increase; perhaps there are other factors affecting price that have not been accounted for. Factors such as the review statistics (cleanliness, check in etc) were left out due to the high colinearity exhibited with other variables of interest, and thus could not be included in the model.
