...
Through skimming the data, we see that there are 23 character, 5 date, 9 logical, and 37 numeric variables. For certain variables like bathrooms and license, all the entries are missing. Therefore, before dropping off the missing values immediately, first we need to eliminate the variables that we do not think will be useful for our analysis. Then, we can drop the missing values for the variables of interest. Additionally, some of the important variables like price is stored as a character. So, we need to convert it into numeric variable before we start building our model (see data wrangling). 
...
#Density Plot for Minimum Nights 
ggplot(new_listings, aes(x= minimum_nights)) +
  geom_density(alpha=0.2) +   
  theme_bw() +                
  labs (
    title = "Density Plot for Minimum Nights",
    x= "Minimum Nights",
    y= "Density")

#Density Plot for Maximum Nights 
ggplot(new_listings, aes(x= maximum_nights)) +
  geom_density(alpha=0.2) +   
  theme_bw() +                
  labs (
    title = "Density Plot for Maximum Nights",
    x= "Maximum Nights",
    y= "Density")

#Density Plot for Availability within next 30 days 
ggplot(new_listings, aes(x= availability_30)) +
  geom_density(alpha=0.2) +   
  theme_bw() +                
  labs (
    title = "Density Plot for Availability Within Next 30 Days",
    x= "Availability within next 30 days",
    y= "Density")

#Density Plot for Availability within next 365 days 
ggplot(new_listings, aes(x= availability_365)) +
  geom_density(alpha=0.2) +   
  theme_bw() +                
  labs (
    title = "Density Plot for Availability Within Next 365 Days",
...
ggplot(new_listings, aes(y= property_type )) +
  geom_bar(alpha=0.2) +   
  theme_bw() +                
  labs (
    title = "Property Type",
...
#Bar chart for Number of Accommodates
...
ggplot(new_listings, aes(x= has_availability)) +
  geom_bar(alpha=0.2) +   
  theme_bw() +                
  labs (
...
```{r}
model1 <- lm(price_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating, data = four_nights_for_two_listings) #change prop_type_simplified into numerical 
...
```{r}
model2 <- lm(price_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating, data = four_nights_for_two_listings + room_type)
